$dialog show @s {\
  ...
  "exit_action": {\
    ...
    "action": {\
      ...
      "template": "function jodek:config/save/$(id) {dialog_load_message:$(dialog_load_message), dialog_advancements:$(dialog_advancements), id:$(id)}"\
    }\
  },\
  "columns": 1,\
  "actions": [\
    {\
      ...
      "action": {\
        ...
        "command": "function $(id):config/dialog_config with storage $(id):root data"\
      }\
    },\
    {\
      "label": {\
      ...
      "action": {\
        "type": "minecraft:run_command",\
        "command": "function jodek:app/uninstall/$(id)"\
      }\
    },\
    {\
      "label": "Modrinth",\
      "action": {\
        ...
        "url": "https://modrinth.com/datapack/$(link)"\
      }\
    }\
  ]\
}